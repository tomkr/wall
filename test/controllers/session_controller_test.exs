defmodule Wall.SessionControllerTest do
  ...
  test "shows a sign in form", %{conn: conn} do
    conn = get conn, session_path(conn, :new)
    assert html_response(conn, 200) =~ "Sign in using your Google Apps account"
  end

  test "logs the user in when providing right credentials", %{conn: conn} do
    conn = post conn, session_path(conn, :create, %{"username" => "test", "password" => "test"})
    assert redirected_to(conn) == "/"
  end

  test "sets a flash error when providing the wrong credentials", %{conn: conn} do
  ...
  test "sets a flash error when providing no credentials at all", %{conn: conn} do
    conn = post conn, session_path(conn, :create)
    assert get_flash(conn)["error"] == "An error occured: No credentials provided."
  end

  test "redirects back to the home page after logging out", %{conn: conn} do
    conn = get conn, session_path(conn, :delete)
    assert redirected_to(conn) =~ "/"
  end

  test "sets a flash confirmation after logging out", %{conn: conn} do
    conn = get conn, session_path(conn, :delete)
